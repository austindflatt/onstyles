...
var getSizeValue = require('../get-size-value/get-size-value.js');
...
function useExtractedMargins({ others, style }) {
  ...
  if (isValidMargin(others.my)) {
    const margin = getSizeValue.getSizeValue({ size: others.my, sizes: theme.spacing });
    mergedStyles.marginTop = margin;
    mergedStyles.marginBottom = margin;
  }
  if (isValidMargin(others.mx)) {
    const margin = getSizeValue.getSizeValue({ size: others.mx, sizes: theme.spacing });
    mergedStyles.marginLeft = margin;
    mergedStyles.marginRight = margin;
  }
  Object.keys(margins).forEach((margin) => {
    if (isValidMargin(others[margin])) {
      mergedStyles[margins[margin]] = getSizeValue.getSizeValue({ size: others[margin], sizes: theme.spacing });
    }
  });
