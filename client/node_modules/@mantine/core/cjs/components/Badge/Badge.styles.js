...
var useStyles = styles.createStyles((theme, { color, size, radius, gradientFrom, gradientTo, gradientDeg, fullWidth }) => {
  const dotSize = styles.getSizeValue({ size, sizes: dotSizes });
  ...
  return {
    ...
    root: __spreadProps(__spreadValues(__spreadValues({}, styles.getFocusStyles(theme)), styles.getFontStyles(theme)), {
      fontSize,
      height,
      ...
      textDecoration: "none",
      padding: `0 ${styles.getSizeValue({ size, sizes: theme.spacing }) / 1.5}px`,
    ...
    dot: {
      ...
      paddingLeft: styles.getSizeValue({ size, sizes: theme.spacing }) / 1.5 - dotSize / 2,
      "&::before": {
        content: '""',
        display: "block",
        width: dotSize,
        ...
        backgroundColor: styles.getThemeColor({
          theme,
          color,
