...
function AvatarsGroup(_a) {
  ...
  const { classes, cx } = AvatarsGroup_styles['default']({ spacing }, { classNames, styles, sx, name: "AvatarsGroup" });
  const avatars = React__default.Children.toArray(children).filter((child) => child.type === Avatar.Avatar).map((child, index) => React__default.cloneElement(child, {
    size,
    radius,
    key: index,
