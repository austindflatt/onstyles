...
const ColorSlider = React.forwardRef((_a, ref) => {
  ...
  const { sxClassName } = styles.useSx({ sx, className });
  const { classes, cx } = ColorSlider_styles['default']({ size }, { classNames, styles: styles$1, name: __staticSelector });
  ...
  const { ref: sliderRef } = hooks.useMove(({ x }) => onChange(getChangeValue(x)));
  hooks.useDidUpdate(() => {
