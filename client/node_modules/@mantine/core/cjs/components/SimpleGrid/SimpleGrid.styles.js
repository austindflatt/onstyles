...
var useStyles = styles.createStyles((theme, { spacing, breakpoints, cols }) => {
  const gridBreakpoints = breakpoints.reduce((acc, breakpoint) => {
    const breakpointSize = styles.getSizeValue({ size: breakpoint.maxWidth, sizes: theme.breakpoints });
    acc[`@media (max-width: ${breakpointSize}px)`] = {
      gridTemplateColumns: `repeat(${breakpoint.cols}, minmax(0, 1fr))`,
      gap: styles.getSizeValue({
  ...
  return {
    root: __spreadValues({
      ...
      gap: styles.getSizeValue({ size: spacing, sizes: theme.spacing })
    }, gridBreakpoints)
  };
});
